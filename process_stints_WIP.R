...
bosupd
match_lineup <- vector("list", length = (n_stints[
[1]]+1))

a2[1,]$data[[1]]

match_lineup[[1]] <- bosline
for (i in 2:n_stints[[1]]){
  print(i)
  match_lineup[[i]] <- match_lineup[[i-1]] %>% left_join(a2[(i-1),]$data[[1]], by = c("player.id" = "substitution.outgoingPlayer.id"))

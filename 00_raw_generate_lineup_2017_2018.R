...
while (length(games_to_download) > 0) {
  ...
  tryCatch({
    games_to_download %>% map(~ API_request_lineup(version, league, season, feed, formato, output =  paste0(output,.), game = .))}, error = function(e){})
  
  
  # Cuales ya baje
  already_downloaded <-  stringr::str_sub(stringr::word(list.files(here::here("data","raw","lineup")),
                                                        1, sep = "\\."), -16, end = -1) 
